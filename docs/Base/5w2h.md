...
A tabela 5W2H foi montada levando em consideração os requisitos funcionais e não funcionais determinados na fase de [Brainstorming](./Base/1.1.1.4.Brainstorming) realizada pelo grupo. Isso foi útil para determinar quais serão as diferentes abordagens de solução do problema em questão. Problema esse que foi mais bem definido na parte de [Entrevista](./Base/1.1.5.1.Entrevista?id=entrevistas-agendafcte) e [Questionário](./Base/1.1.5.2.Questionario?id=questionário), onde pessoas do publico alvo do projeto deram seu input a solução.
